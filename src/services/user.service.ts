...
export const updateUser = async (_id: string, update: UserDocument) => {
  try {
    const user = await UserModel.findByIdAndUpdate(_id, update).select(
      "-password"
    );
  } catch (error: any) {
    throw new Error(error);
...
export const login = async (username: string, password: string) => {
  try {
    ...
    const user = pick(userDoc, ["username", "_id", "chats", "contacts"]);
